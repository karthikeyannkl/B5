...
function freshDB(){return {adminPassword:'ADMIN',members:[],pins:[],messages:[],upgrades:[],transactions:[]};}
...
let db=load();
db.upgrades=db.upgrades||[]; db.transactions=db.transactions||[];
db.members.forEach(m=>{if(!m.level)m.level=1});
...
function memberPublic(m){return {memberId:m.memberId,name:m.name,mobile:m.mobile,status:m.status,referral:m.referral,level:m.level||1}}
...
app.post('/api/member/register',(req,res)=>{
 ...
 const m={...b,memberId:id(),status:'Pending',level:1,registeredAt:new Date().toISOString()};
...
// ---------------- LEVEL TRACKING / SEQUENTIAL UPGRADE ----------------
const LEVEL_MILESTONES={1:3,2:6,3:9,4:12,5:15,6:18};
function directReferrals(memberId){return db.members.filter(m=>m.referral===memberId)}
function levelTrackState(memberId){
 const m=db.members.find(x=>x.memberId===memberId); if(!m)return null;
 const refs=directReferrals(memberId); const level=m.level||1; const need=LEVEL_MILESTONES[level]||null;
 const upgrade=db.upgrades.find(u=>u.payerMemberId===memberId && u.status!=='FINAL_APPROVED');
 const incoming=db.upgrades.find(u=>u.receiverMemberId===memberId && u.status!=='FINAL_APPROVED');
 const history=db.upgrades.filter(u=>u.payerMemberId===memberId && u.status==='FINAL_APPROVED').map(u=>({from:u.from,to:u.to,payerMemberId:u.payerMemberId,receiverMemberId:u.receiverMemberId,amount:u.payment?.amount||'',utr:u.utr||'',approvedAt:u.approvedAt}));
 const earnings=db.transactions.filter(t=>t.receiverMemberId===memberId).map(t=>({level:t.level,fromMemberId:t.fromMemberId,amount:t.amount,status:t.status,utr:t.utr,at:t.at}));
 return {member:memberPublic(m),directReferrals:refs.map(memberPublic),directReferralCount:refs.length,need,nextLevel:level<7?level+1:null,eligible:level<7&&!upgrade&&refs.length>=(LEVEL_MILESTONES[level]||999),upgrade:upgrade||null,incoming:incoming||null,history,earnings};
}
app.get('/api/member/leveltrack/:id',(req,res)=>{const d=levelTrackState(req.params.id);if(!d)return res.status(404).json({error:'Member not found'});res.json(d)});
app.post('/api/member/leveltrack/request',(req,res)=>{
 const m=db.members.find(x=>x.memberId===req.body.memberId); if(!m)return res.status(404).json({error:'Member not found'});
 const level=m.level||1, refs=directReferrals(m.memberId).length, need=LEVEL_MILESTONES[level];
 if(level>=7)return res.status(400).json({error:'Level 7 is final level'});
 if(db.upgrades.some(u=>u.payerMemberId===m.memberId&&u.status!=='FINAL_APPROVED'))return res.status(400).json({error:'Upgrade already in progress'});
 if(refs<need)return res.status(400).json({error:`Need ${need} direct referrals for Level ${level+1}`});
 const u={id:'UPG-'+Date.now()+'-'+crypto.randomBytes(2).toString('hex').toUpperCase(),payerMemberId:m.memberId,payerName:m.name,from:level,to:level+1,status:'REQUESTED',payment:null,utr:'',receiverConfirmed:false,requestedAt:new Date().toISOString()};
 db.upgrades.push(u);save(db);res.json({ok:true,state:levelTrackState(m.memberId)});
});
app.get('/api/admin/leveltrack',(req,res)=>{
 const active=db.upgrades.filter(u=>u.status!=='FINAL_APPROVED').map(u=>{const m=db.members.find(x=>x.memberId===u.payerMemberId);return {...u,payerName:m?.name||u.payerName,directReferralCount:m?directReferrals(m.memberId).length:0}});
 res.json({requests:active,history:db.upgrades.filter(u=>u.status==='FINAL_APPROVED').slice(-100).reverse()});
});
app.post('/api/admin/leveltrack/payment-details',(req,res)=>{
 const u=db.upgrades.find(x=>x.id===req.body.upgradeId); if(!u)return res.status(404).json({error:'Upgrade request not found'});
 const receiver=db.members.find(x=>x.memberId===String(req.body.receiverMemberId||'')); if(!receiver)return res.status(404).json({error:'Receiver Member ID not found'});
 u.payment={receiverName:receiver.name,receiverMemberId:receiver.memberId,receiverPhone:receiver.mobile,accountHolder:String(req.body.accountHolder||receiver.account||receiver.name),bankName:String(req.body.bankName||receiver.bank||''),accountNumber:String(req.body.accountNumber||receiver.account||''),ifsc:String(req.body.ifsc||receiver.ifsc||''),upi:String(req.body.upi||''),amount:String(req.body.amount||''),adminMessage:String(req.body.adminMessage||'')};
 u.receiverMemberId=receiver.memberId; u.status='PAYMENT_DETAILS_SENT'; u.detailsSentAt=new Date().toISOString(); save(db); res.json({ok:true,state:levelTrackState(u.payerMemberId)});
});
app.post('/api/member/leveltrack/utr',(req,res)=>{
 const u=db.upgrades.find(x=>x.id===req.body.upgradeId); if(!u||u.payerMemberId!==req.body.memberId)return res.status(404).json({error:'Upgrade request not found'});
 if(!u.payment)return res.status(400).json({error:'Admin payment details are not sent yet'});
 const utr=String(req.body.utr||'').trim(); if(!utr)return res.status(400).json({error:'UTR is required'});
 u.utr=utr;u.status='UTR_SUBMITTED';u.utrAt=new Date().toISOString();save(db);res.json({ok:true,state:levelTrackState(u.payerMemberId)});
});
app.post('/api/member/leveltrack/receiver-confirm',(req,res)=>{
 const u=db.upgrades.find(x=>x.id===req.body.upgradeId); if(!u)return res.status(404).json({error:'Upgrade request not found'});
 if(u.receiverMemberId!==req.body.receiverMemberId)return res.status(403).json({error:'Receiver Member ID mismatch'});
 if(!u.utr)return res.status(400).json({error:'UTR not submitted yet'});
 u.receiverConfirmed=true;u.status='RECEIVER_CONFIRMED';u.receiverConfirmedAt=new Date().toISOString();save(db);res.json({ok:true});
});
app.post('/api/member/leveltrack/receiver-reject',(req,res)=>{
 const u=db.upgrades.find(x=>x.id===req.body.upgradeId); if(!u)return res.status(404).json({error:'Upgrade request not found'});
 if(u.receiverMemberId!==req.body.receiverMemberId)return res.status(403).json({error:'Receiver Member ID mismatch'});
 u.receiverConfirmed=false;u.status='PAYMENT_NOT_RECEIVED';u.receiverRejectedAt=new Date().toISOString();save(db);res.json({ok:true});
});
app.post('/api/admin/leveltrack/final-approve',(req,res)=>{
 const u=db.upgrades.find(x=>x.id===req.body.upgradeId); if(!u)return res.status(404).json({error:'Upgrade request not found'});
 if(!u.receiverConfirmed||!u.utr)return res.status(400).json({error:'Receiver confirmation and UTR are required'});
 const m=db.members.find(x=>x.memberId===u.payerMemberId); if(!m)return res.status(404).json({error:'Payer member not found'});
 m.level=u.to; u.status='FINAL_APPROVED';u.approvedAt=new Date().toISOString();
 db.transactions.push({type:'UPGRADE_PAYMENT',level:u.to,fromMemberId:u.payerMemberId,receiverMemberId:u.receiverMemberId,amount:u.payment?.amount||'',utr:u.utr,status:'COMPLETED',at:new Date().toISOString(),upgradeId:u.id});
 save(db); res.json({ok:true,state:levelTrackState(m.memberId),member:memberPublic(m)});
});
