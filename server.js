...
// Permanent database: when DATABASE_URL is configured, PostgreSQL is the source of truth.
// If it is unavailable, the server will NOT start, preventing silent data loss on redeploy.
...
function save(d){
  ...
  if(pool){
    pool.query('UPDATE app_state SET data=$1 WHERE id=1',[JSON.stringify(d)])
...
async function initPersistentDatabase(){
  if(!pool) throw new Error('DATABASE_URL is not configured. Permanent database is required; server will not start without it.');
  await pool.query('CREATE TABLE IF NOT EXISTS app_state (id INTEGER PRIMARY KEY, data JSONB NOT NULL)');
  const r=await pool.query('SELECT data FROM app_state WHERE id=1');
  if(r.rows.length){
    db=typeof r.rows[0].data==='string'?JSON.parse(r.rows[0].data):r.rows[0].data;
    console.log('Persistent database loaded; existing data preserved');
  }else{
    await pool.query('INSERT INTO app_state(id,data) VALUES(1,$1)',[JSON.stringify(db)]);
    console.log('Persistent database initialized from existing local data');
  }
}
