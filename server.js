...
function normalizeDB(d){
  d=d||freshDB();
  d.schemaVersion=4;
  d.adminPassword=d.adminPassword||'ADMIN';
  d.members=d.members||[]; d.pins=d.pins||[]; d.messages=d.messages||[]; d.passwordResetRequests=d.passwordResetRequests||[];
  d.leveltrackRequests=d.leveltrackRequests||[]; d.leveltrackUpgrades=d.leveltrackUpgrades||[]; d.leveltrackPayments=d.leveltrackPayments||[]; d.leveltrackMessages=d.leveltrackMessages||[];
  d.paymentSettings=d.paymentSettings||{};
  d.paymentSettings.admins=Array.isArray(d.paymentSettings.admins)&&d.paymentSettings.admins.length?d.paymentSettings.admins:[
    {id:'A',name:'Admin A',accountHolder:'',bank:'',account:'',ifsc:'',upi:'',active:true},
    {id:'B',name:'Admin B',accountHolder:'',bank:'',account:'',ifsc:'',upi:'',active:true},
    {id:'C',name:'Admin C',accountHolder:'',bank:'',account:'',ifsc:'',upi:'',active:true}
  ];
  d.paymentSettings.adminRotationIndex=Number(d.paymentSettings.adminRotationIndex||0);
  d.paymentSettings.trust=d.paymentSettings.trust||{name:'Registered Trust',accountHolder:'',bank:'',account:'TEMP-TRUST-001',ifsc:'',upi:'',active:true};
  if(!d.pins.some(x=>(x.pin==='PIN-START1'||x.pin==='B5-FMUXNF')&&x.status==='AVAILABLE')){
    const old=d.pins.find(x=>x.pin==='PIN-START1')||d.pins.find(x=>x.pin==='B5-FMUXNF');
    if(!old)d.pins.push({pin:'PIN-START1',assignedTo:null,status:'AVAILABLE',usedBy:null,createdAt:new Date().toISOString(),system:true});
    else if(!old.usedBy){old.status='AVAILABLE';old.assignedTo=null;}
  }
  d.members.forEach(m=>{if(!m.referralId)m.referralId=referralCode(m.memberId)});
  return d;
}
...
async function initStore(){
  if(process.env.DATABASE_URL){
    ...
    db=r.rows[0]?.state ? normalizeDB(r.rows[0].state) : normalizeDB(loadLocal());
    await pgPool.query('INSERT INTO borntown5_state(id,schema_version,state,updated_at) VALUES(1,$1,$2,NOW()) ON CONFLICT(id) DO UPDATE SET schema_version=EXCLUDED.schema_version,state=EXCLUDED.state,updated_at=NOW()',[db.schemaVersion,db]);
    console.log('BORNTOWIN5 datastore: PostgreSQL');
  } else {
    db=normalizeDB(loadLocal());
    fs.writeFileSync(DB_FILE,JSON.stringify(db,null,2));
    console.log('BORNTOWIN5 datastore: local-file (set DATABASE_URL for production persistence)');
  }
}

function save(d){
  const snapshot=JSON.parse(JSON.stringify(d));
  if(!pgPool){fs.writeFileSync(DB_FILE,JSON.stringify(snapshot,null,2));fs.writeFileSync(BACKUP_FILE,JSON.stringify(snapshot,null,2));return Promise.resolve();}
  saveQueue=saveQueue.then(()=>pgPool.query('UPDATE borntown5_state SET schema_version=$1,state=$2,updated_at=NOW() WHERE id=1',[snapshot.schemaVersion||4,snapshot]));
  saveQueue=saveQueue.catch(err=>{console.error('Database save failed:',err.message)});
  return saveQueue;
}

let db;

function hashPassword(v,salt=crypto.randomBytes(16).toString('hex')){return salt+':'+crypto.scryptSync(String(v||''),salt,64).toString('hex')}
function verifyPassword(v,stored){if(!stored)return false;const raw=String(stored);if(raw.includes(':')){const [salt,hash]=raw.split(':');try{const actual=crypto.scryptSync(String(v||''),salt,64).toString('hex');return crypto.timingSafeEqual(Buffer.from(actual,'hex'),Buffer.from(hash,'hex'))}catch(e){return false}}return crypto.createHash('sha256').update(String(v||'')).digest('hex')===raw}
...
app.post('/api/member/login',async (req,res)=>{const m=db.members.find(x=>x.mobile===String(req.body.mobile||'').trim());if(!m)return res.status(404).json({error:'Member not found. Please register first.'});const supplied=String(req.body.password||'');if(!m.passwordHash)return res.status(401).json({error:'Password not set. Please use Forgot Password and contact Admin.'});if(!verifyPassword(supplied,m.passwordHash))return res.status(401).json({error:'Incorrect password'});res.json({member:{...memberPublic(m),mustChangePassword:!!m.mustChangePassword}})});
app.post('/api/member/forgot-password',async (req,res)=>{const mobile=String(req.body.mobile||'').trim();const m=db.members.find(x=>x.mobile===mobile);if(!m)return res.status(404).json({error:'Mobile number not found'});const pending=db.passwordResetRequests.find(x=>x.memberId===m.memberId&&x.status==='Pending');if(pending)return res.json({ok:true,message:'Password reset request already sent to Admin.'});db.passwordResetRequests.push({id:'PR-'+crypto.randomBytes(4).toString('hex').toUpperCase(),memberId:m.memberId,mobile:m.mobile,name:m.name,status:'Pending',createdAt:new Date().toISOString()});await save(db);res.json({ok:true,message:'Password reset request sent to Admin. Please contact Admin after approval.'})});
app.post('/api/member/password/:id',async (req,res)=>{const m=db.members.find(x=>x.memberId===req.params.id);if(!m)return res.status(404).json({error:'Member not found'});if(!m.passwordHash||!verifyPassword(req.body.oldPassword,m.passwordHash))return res.status(401).json({error:'Current password is incorrect'});const np=String(req.body.newPassword||'');if(np.length<6)return res.status(400).json({error:'New password must be at least 6 characters'});m.passwordHash=hashPassword(np);m.mustChangePassword=false;await save(db);res.json({ok:true})});
...
app.get('/api/member/profile/:id',async (req,res)=>{const m=db.members.find(x=>x.memberId===req.params.id);if(!m)return res.status(404).json({error:'Member not found'});res.json({profile:memberProfile(m)})});
app.post('/api/member/profile/:id',async (req,res)=>{const m=db.members.find(x=>x.memberId===req.params.id);if(!m)return res.status(404).json({error:'Member not found'});if(m.profileLocked)return res.status(403).json({error:'Profile is locked. Please contact Admin for changes.'});const b=req.body||{};for(const k of ['name','place','accountHolder','bank','account','ifsc','branch','upi'])if(b[k]!==undefined)m[k]=String(b[k]).trim();if(b.photo!==undefined){const photo=String(b.photo);if(photo.length>1200000)return res.status(400).json({error:'Profile photo is too large'});m.photo=photo}if(!m.name||!m.place||!m.accountHolder||!m.bank||!m.account||!m.ifsc||!m.branch||!m.upi)return res.status(400).json({error:'Please complete all profile and bank details'});m.profileLocked=true;await save(db);res.json({ok:true,member:memberPublic(m),profile:memberProfile(m)})});
...
const LEVEL_RULES={
  1:{upgrade:1000,member:1000,trust:0,admin:0,required:5},
  2:{upgrade:3000,member:3000,trust:0,admin:0,required:10},
  3:{upgrade:20000,member:15600,trust:2400,admin:2000,required:10},
  4:{upgrade:100000,member:72000,trust:16000,admin:12000,required:5},
  5:{upgrade:200000,member:148000,trust:32000,admin:20000,required:5},
  6:{upgrade:500000,member:240000,trust:160000,admin:100000,required:5},
  7:{upgrade:0,member:0,trust:0,admin:0,required:0}
};
...
app.get('/api/leveltrack/admin/next-receiver/:level',async (req,res)=>{
...
app.post('/api/leveltrack/member/incoming/:id/accept',async (req,res)=>{
  ...
  p.memberAccepted=true;p.acceptedAt=new Date().toISOString();const mp=(p.parts||[]).find(x=>x.type==='member');if(mp)mp.status='CONFIRMED';const u=db.leveltrackUpgrades.find(x=>x.id===p.upgradeId);if(u)u.receiverApproved=true;await save(db);res.json({ok:true,payment:p});
});
...
app.get('/api/health',async (req,res)=>res.json({ok:true,service:'BORNTOWIN5',schemaVersion:db?.schemaVersion||4,dataStore:pgPool?'postgresql':'local-file',members:db?.members?.length||0,pins:db?.pins?.length||0,messages:db?.messages?.length||0,leveltrackRequests:db?.leveltrackRequests?.length||0,leveltrackPayments:db?.leveltrackPayments?.length||0}));
app.post('/api/admin/backup/export',async (req,res)=>{if(String(req.body?.password||'')!==String(db.adminPassword||''))return res.status(401).json({error:'Admin password required'});res.setHeader('Content-Type','application/json');res.setHeader('Content-Disposition',`attachment; filename="borntown5-backup-${new Date().toISOString().replace(/[:.]/g,'-')}.json"`);res.send(JSON.stringify(db,null,2));});

const PORT=process.env.PORT||10000;
initStore().then(()=>{app.listen(PORT,'0.0.0.0',()=>console.log('BORNTOWIN5 running on '+PORT));}).catch(err=>{console.error('BORNTOWIN5 startup failed:',err);process.exit(1)});
