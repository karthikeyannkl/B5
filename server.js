...
if(process.env.DATABASE_URL){
  try{
    ...
    console.log('DATABASE_URL diagnostic (channel binding disabled):',JSON.stringify({
      ...
      passwordHasLeadingOrTrailingWhitespace:/^\s|\s$/.test(rawPassword),
      sslmode:u.searchParams.get('sslmode')||'(none)',
      channelBinding:u.searchParams.get('channel_binding')||'(none)'
    }));
  }catch(e){
    console.error('DATABASE_URL diagnostic: INVALID URL:',e.message);
  }
}else{
  console.log('DATABASE_URL diagnostic: NOT CONFIGURED');
}
...
function ltTree(id){
  ...
  const make=m=>({memberId:m.memberId,name:m.name,mobile:m.mobile,email:m.email||m.rEmail||'',status:m.status,level:Number(m.level||1),levelMemberId:m.levelMemberId||null,children:direct(m.memberId).map(make)});
  return make(root);
}
...
function ltDashboard(memberId){
  ...
  const msgs=[...db.messages.filter(x=>x.to===memberId||x.to==='ALL'),...db.leveltrackMessages.filter(x=>x.to===memberId)].sort((a,b)=>String(b.at).localeCompare(String(a.at))).slice(0,2).map(x=>({message:x.message,at:x.at}));
...
app.post('/api/leveltrack/admin/upgrades/:id/final-approve',(req,res)=>{
  ...
  db.leveltrackMessages.push({to:m.memberId,message:`Level upgrade completed. You are now Level ${m.level}.`,at:new Date().toISOString()});save(db);sendEmail(m.email||m.rEmail,'BORNTOWIN5 - Level Upgrade Completed',`Hello ${m.name},\n\nYour Level upgrade is completed successfully.\nYou are now Level ${m.level}.`).catch(e=>console.error('Level upgrade email failed:',e.message));res.json({ok:true,member:memberPublic(m),upgrade:u});
});
...
app.post('/api/leveltrack/member/incoming/:id/accept',(req,res)=>{
  ...
  p.memberAccepted=true;p.acceptedAt=new Date().toISOString();const mp=(p.parts||[]).find(x=>x.type==='member');if(mp)mp.status='CONFIRMED';const u=db.leveltrackUpgrades.find(x=>x.id===p.upgradeId);if(u)u.receiverApproved=true;save(db);const upgrader=u?ltMember(u.memberId):null;if(upgrader)sendEmail(upgrader.email||upgrader.rEmail,'BORNTOWIN5 - Payment Accepted',`Your payment receiver has accepted the payment for Level ${u.from} to Level ${u.to}.`).catch(e=>console.error('Payment accepted email failed:',e.message));res.json({ok:true,payment:p});
});
app.get('/api/leveltrack/member/messages/:id',(req,res)=>{
  const msgs=[...db.messages.filter(x=>x.to===req.params.id||x.to==='ALL').map(x=>({message:x.message,at:x.at})),...db.leveltrackMessages.filter(x=>x.to===req.params.id).map(x=>({message:x.message,at:x.at}))].sort((a,b)=>String(b.at).localeCompare(String(a.at))).slice(0,2);
  res.json({messages:msgs});
});

const PORT=process.env.PORT||10000;
initPersistentDatabase().then(()=>app.listen(PORT,'0.0.0.0',()=>console.log('BORNTOWIN5 running on '+PORT))).catch(err=>{console.error('Persistent database initialization failed:',err.message);process.exit(1)});
